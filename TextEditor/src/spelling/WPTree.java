...
public class WPTree implements WordPath {
	...
	/*public List<String> findPath(String word1, String word2) 
	{
		...
		WPTreeNode currNode = new WPTreeNode(word1, root);
		...
		while(!visited.contains(word2)) // wrong condition, temporary
		{
			String currWord = queue.remove(0);
			
			for( String s : nw.distanceOne(currWord,true) )
			{
				...
				if(!visited.contains(s))
				{
					...
					if(s.equals(word2)) List<String> ret = 
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		while( !queue.isEmpty() && !visited.contains(word2))
		{
			...
			for(String s : nw.distanceOne(curr.getWord(), true))
			{
				if(!visited.contains(s))
				{
					WPTreeNode x = curr.addChild(s);
					queue.add(x);
					visited.add(s);
					if( s.equals(word2) ) p = x.buildPathToRoot();
				}
			}
